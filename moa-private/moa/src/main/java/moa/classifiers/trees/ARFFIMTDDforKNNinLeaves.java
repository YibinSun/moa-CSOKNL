...
/**
 * Implementation of ARFFIMTDDforKNNinLeaves, an extension of FIMTDD to be used by AdaptiveRandomForestRegressor.
 *
 ...
 */
public class ARFFIMTDDforKNNinLeaves extends AbstractClassifier implements Regressor{

    private static final long serialVersionUID = 1L;

    protected ARFFIMTDDforKNNinLeaves.Node treeRoot;
    ...
    // YS      New Features
    public ArrayList<ARFFIMTDDforKNNinLeaves.LeafNode> leafNodes;

    /** YS     Occupied Characters for Options: k s g c t a h f y u l d p **/
    ...
    public abstract static class Node extends AbstractMOAObject {
        ...
        protected ARFFIMTDDforKNNinLeaves tree;

        protected boolean changeDetection = true;

        protected ARFFIMTDDforKNNinLeaves.Node parent;

        protected ARFFIMTDDforKNNinLeaves.Node alternateTree;
        protected ARFFIMTDDforKNNinLeaves.Node originalNode;
        ...
        public Node(ARFFIMTDDforKNNinLeaves tree) {
            this.tree = tree;
            ID = tree.maxID;
        }

        public void copyStatistics(ARFFIMTDDforKNNinLeaves.Node node) {
         ...
         * Set the parent node
         */
        public void setParent(ARFFIMTDDforKNNinLeaves.Node parent) {
            this.parent = parent;
        }
         ...
         */
        public ARFFIMTDDforKNNinLeaves.Node getParent() {
            return parent;
        }
        ...
        public int getLevel() {
            ARFFIMTDDforKNNinLeaves.Node target = this;
            int level = 0;
        ...
        public void setChild(int parentBranch, ARFFIMTDDforKNNinLeaves.Node node) {
        }

        public int getChildIndex(ARFFIMTDDforKNNinLeaves.Node child) {
            return -1;
        }
    ...
    public static class LeafNode extends ARFFIMTDDforKNNinLeaves.Node {

        private static final long serialVersionUID = 1L;

        // Perceptron model that carries out the actual learning in each node
        public ARFFIMTDDforKNNinLeaves.FIMTDDPerceptron learningModel;
        ...
        // YS     ArrayList to store all the instances
        public ArrayList<Instance> leafInstances;

        /**
         * Create a new LeafNode
         */
        public LeafNode(ARFFIMTDDforKNNinLeaves tree, int subspaceSize) {
            super(tree);
            examplesSeen = 0;
            sumOfValues = 0;
            sumOfSquares = 0;
            ...
            // YS      Set the leafInstances
            this.leafInstances = new ArrayList<>();
        }
         ...
         * attribute observer
         */
        public void learnFromInstance(Instance inst, boolean growthAllowed, ARFFIMTDDforKNNinLeaves ARFFIMTDDforKNNinLeaves) {
            ...
            if (this.listAttributes == null) {
                ...
                for (int j = 0; j < this.numAttributes; j++) {
                    ...
                    while (isUnique == false) {
                        this.listAttributes[j] = ARFFIMTDDforKNNinLeaves.classifierRandom.nextInt(inst.numAttributes() - 1);
                        isUnique = true;
                        for (int i = 0; i < j; i++) {
...
//                        }
        ...
        public void checkForSplit(ARFFIMTDDforKNNinLeaves tree) {
    ...
    public abstract static class InnerNode extends ARFFIMTDDforKNNinLeaves.Node {
        // The InnerNode and SplitNode design is used for easy extension in ORTO
        private static final long serialVersionUID = 1L;

        protected AutoExpandVector<ARFFIMTDDforKNNinLeaves.Node> children = new AutoExpandVector<ARFFIMTDDforKNNinLeaves.Node>();
        ...
        public InnerNode(ARFFIMTDDforKNNinLeaves tree) {
            super(tree);
        }
        ...
        public ARFFIMTDDforKNNinLeaves.Node getChild(int index) {
        ...
        public void setChild(int index, ARFFIMTDDforKNNinLeaves.Node child) {
            children.set(index, child);
        }

        public void disableChangeDetection() {
            changeDetection = false;
            for (ARFFIMTDDforKNNinLeaves.Node child : children) {
    ...
    public static class SplitNode extends ARFFIMTDDforKNNinLeaves.InnerNode {
         ...
         * @param tree
         */
        public SplitNode(InstanceConditionalTest splitTest, ARFFIMTDDforKNNinLeaves tree) {
            super(tree);
        ...
        public ARFFIMTDDforKNNinLeaves.Node descendOneStep(Instance inst) {
            return children.get(splitTest.branchForInstance(inst));
        }

        public void describeSubtree(StringBuilder out, int indent) {
            for (int branch = 0; branch < children.size(); branch++) {
                ARFFIMTDDforKNNinLeaves.Node child = getChild(branch);
                if (child != null) {
    ...
    public class FIMTDDPerceptron implements Serializable {
        ...
        public FIMTDDPerceptron(ARFFIMTDDforKNNinLeaves.FIMTDDPerceptron original) {
            this.tree = original.tree;
            weightAttribute = (DoubleVector) original.weightAttribute.copy();
            reset = false;
        }

        public FIMTDDPerceptron(ARFFIMTDDforKNNinLeaves tree) {
            this.tree = tree;
            reset = true;
        }
        ...
        private double denormalizePrediction(double normalizedPrediction, ARFFIMTDDforKNNinLeaves tree) {
    ...
    public void processInstance(Instance inst, ARFFIMTDDforKNNinLeaves.Node node, double prediction, double normalError, boolean growthAllowed, boolean inAlternate) {
        ARFFIMTDDforKNNinLeaves.Node currentNode = node;
        while (true) {
            if (currentNode instanceof ARFFIMTDDforKNNinLeaves.LeafNode) {
                // YS     Add Instances into LeafNodes
//                for(int i = 0; i < inst.weight(); i++)
                ((ARFFIMTDDforKNNinLeaves.LeafNode) currentNode).leafInstances.add(inst);


                ((ARFFIMTDDforKNNinLeaves.LeafNode) currentNode).learnFromInstance(inst, growthAllowed, this);

                break;
            } else {
                currentNode.examplesSeen += inst.weight();
                currentNode.sumOfAbsErrors += inst.weight() * normalError;
                ARFFIMTDDforKNNinLeaves.SplitNode iNode = (ARFFIMTDDforKNNinLeaves.SplitNode) currentNode;
                if (!inAlternate && iNode.alternateTree != null) {
                    ...
                    if (iNode.lossExamplesSeen - iNode.previousWeight >= alternateTreeTMinOption.getValue()) {
                        ...
                        if (Qi > 0) {
                            // Switch the subtrees
                            ARFFIMTDDforKNNinLeaves.Node parent = currentNode.getParent();

                            if (parent != null) {
                                ARFFIMTDDforKNNinLeaves.Node replacementTree = iNode.alternateTree;
                ...
                if (currentNode instanceof ARFFIMTDDforKNNinLeaves.SplitNode) {
                    currentNode = ((ARFFIMTDDforKNNinLeaves.SplitNode) currentNode).descendOneStep(inst);
                }
            }
        }
    }
    ...
    protected ARFFIMTDDforKNNinLeaves.SplitNode newSplitNode(InstanceConditionalTest splitTest) {
        maxID++;
        return new ARFFIMTDDforKNNinLeaves.SplitNode(splitTest, this);
    }

    protected ARFFIMTDDforKNNinLeaves.LeafNode newLeafNode() {
        maxID++;
        return new ARFFIMTDDforKNNinLeaves.LeafNode(this, this.subspaceSizeOption.getValue());
    }

    protected ARFFIMTDDforKNNinLeaves.FIMTDDPerceptron newLeafModel() {
        return new ARFFIMTDDforKNNinLeaves.FIMTDDPerceptron(this);
    }
    ...
    protected void checkRoot() {
        if (treeRoot == null) {
            ...
            leafNodes.add((ARFFIMTDDforKNNinLeaves.LeafNode) treeRoot);
            leafNodeCount = 1;
        }
    }
    ...
    protected void attemptToSplit(ARFFIMTDDforKNNinLeaves.LeafNode node, ARFFIMTDDforKNNinLeaves.Node parent, int parentIndex) {
        ...
        if (shouldSplit) {
            ...
            ARFFIMTDDforKNNinLeaves.SplitNode newSplit = newSplitNode(splitDecision.splitTest);
            newSplit.copyStatistics(node);
            newSplit.changeDetection = node.changeDetection;
            newSplit.ID = node.ID;
            leafNodeCount--;
            for (int i = 0; i < splitDecision.numSplits(); i++) {
                ARFFIMTDDforKNNinLeaves.LeafNode newChild = newLeafNode();
                newChild.changeDetection = node.changeDetection;
                newChild.setParent(newSplit);
                newSplit.setChild(i, newChild);
                leafNodeCount++;
                // YS      Separate Instances into Different Branches
                for(int j = 0; j < node.leafInstances.size(); j++){
                    Instance currentInstance = node.leafInstances.get(j);
                    if (splitDecision.splitTest.branchForInstance(currentInstance) == i)
                        newChild.leafInstances.add(currentInstance);
                }
                // YS      Add New LeafNode into leafNodes
                this.leafNodes.add(newChild);
            }
            // YS      Drop Old Leaves from leafNodes
            if(leafNodes.contains(node)) this.leafNodes.remove(node);
            // YS      Clear Old Leaves to Save Memory
            node.leafInstances = null;
            ...
            if (parent == null && node.originalNode == null) {
                ...
                ((ARFFIMTDDforKNNinLeaves.SplitNode) parent).setChild(parentIndex, newSplit);
    ...
    // YS     New Method to Gain The Leaf That An Instance Belongs to
    public ARFFIMTDDforKNNinLeaves.Node getLeafForInstance(Instance inst, ARFFIMTDDforKNNinLeaves.Node node){
        if(node == null) return null;

        if(node instanceof ARFFIMTDDforKNNinLeaves.LeafNode) return node;
        else{
            ARFFIMTDDforKNNinLeaves.SplitNode currentNode = (ARFFIMTDDforKNNinLeaves.SplitNode)node;
            return getLeafForInstance(inst,currentNode.children.get(currentNode.splitTest.branchForInstance(inst)));
        }
    }

    public ARFFIMTDDforKNNinLeaves.Node getTreeRoot(){
