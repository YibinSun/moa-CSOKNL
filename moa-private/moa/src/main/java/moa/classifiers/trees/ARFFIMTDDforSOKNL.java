 ...
 */
public class ARFFIMTDDforSOKNL extends AbstractClassifier implements Regressor {

    private static final long serialVersionUID = 1L;

    protected ARFFIMTDDforSOKNL.Node treeRoot;
    ...
    // YS      New Features
    public ArrayList<ARFFIMTDDforSOKNL.LeafNode> leafNodes;
    ...
    public abstract static class Node extends AbstractMOAObject {
        ...
        protected ARFFIMTDDforSOKNL tree;

        protected boolean changeDetection = true;

        protected ARFFIMTDDforSOKNL.Node parent;

        protected ARFFIMTDDforSOKNL.Node alternateTree;
        protected ARFFIMTDDforSOKNL.Node originalNode;
        ...
        public Node(ARFFIMTDDforSOKNL tree) {
            this.tree = tree;
            ID = tree.maxID;
        }

        public void copyStatistics(ARFFIMTDDforSOKNL.Node node) {
         ...
         * Set the parent node
         */
        public void setParent(ARFFIMTDDforSOKNL.Node parent) {
            this.parent = parent;
        }
         ...
         */
        public ARFFIMTDDforSOKNL.Node getParent() {
            return parent;
        }
        ...
        public int getLevel() {
            ARFFIMTDDforSOKNL.Node target = this;
            int level = 0;
        ...
        public void setChild(int parentBranch, ARFFIMTDDforSOKNL.Node node) {
        }

        public int getChildIndex(ARFFIMTDDforSOKNL.Node child) {
            return -1;
        }
    ...
    public static class LeafNode extends ARFFIMTDDforSOKNL.Node {

        private static final long serialVersionUID = 1L;

        // Perceptron model that carries out the actual learning in each node
        public ARFFIMTDDforSOKNL.FIMTDDPerceptron learningModel;
...
//        public ArrayList<Instance> leafInstances;
         ...
         */
        public LeafNode(ARFFIMTDDforSOKNL tree, int subspaceSize) {
            super(tree);
            examplesSeen = 0;
            sumOfValues = 0;
            sumOfSquares = 0;
...
//            this.leafInstances = new ArrayList<>();
         ...
         * attribute observer
         */
        public void learnFromInstance(Instance inst, boolean growthAllowed, ARFFIMTDDforSOKNL ARFFIMTDDforSOKNL, ARFFIMTDDforSOKNL.LeafNode node) {
            ...
            //YS     Update the centroid related statistics
            ...
            if (this.listAttributes == null) {
                ...
                for (int j = 0; j < this.numAttributes; j++) {
                    ...
                    while (isUnique == false) {
                        this.listAttributes[j] = ARFFIMTDDforSOKNL.classifierRandom.nextInt(inst.numAttributes() - 1);
                        isUnique = true;
                        for (int i = 0; i < j; i++) {
...
//                        }
        ...
        public void checkForSplit(ARFFIMTDDforSOKNL tree) {
    ...
    public abstract static class InnerNode extends ARFFIMTDDforSOKNL.Node {
        // The InnerNode and SplitNode design is used for easy extension in ORTO
        private static final long serialVersionUID = 1L;

        protected AutoExpandVector<ARFFIMTDDforSOKNL.Node> children = new AutoExpandVector<ARFFIMTDDforSOKNL.Node>();
        ...
        public InnerNode(ARFFIMTDDforSOKNL tree) {
            super(tree);
        }
        ...
        public ARFFIMTDDforSOKNL.Node getChild(int index) {
        ...
        public void setChild(int index, ARFFIMTDDforSOKNL.Node child) {
            children.set(index, child);
        }

        public void disableChangeDetection() {
            changeDetection = false;
            for (ARFFIMTDDforSOKNL.Node child : children) {
    ...
    public static class SplitNode extends ARFFIMTDDforSOKNL.InnerNode {
         ...
         * @param tree
         */
        public SplitNode(InstanceConditionalTest splitTest, ARFFIMTDDforSOKNL tree) {
            super(tree);
        ...
        public ARFFIMTDDforSOKNL.Node descendOneStep(Instance inst) {
            return children.get(splitTest.branchForInstance(inst));
        }

        public void describeSubtree(StringBuilder out, int indent) {
            for (int branch = 0; branch < children.size(); branch++) {
                ARFFIMTDDforSOKNL.Node child = getChild(branch);
                if (child != null) {
    ...
    public class FIMTDDPerceptron implements Serializable {
        ...
        public FIMTDDPerceptron(ARFFIMTDDforSOKNL.FIMTDDPerceptron original) {
            this.tree = original.tree;
            weightAttribute = (DoubleVector) original.weightAttribute.copy();
            reset = false;
        }

        public FIMTDDPerceptron(ARFFIMTDDforSOKNL tree) {
            this.tree = tree;
            reset = true;
        }
        ...
        private double denormalizePrediction(double normalizedPrediction, ARFFIMTDDforSOKNL tree) {
    ...
    public void processInstance(Instance inst, ARFFIMTDDforSOKNL.Node node, double prediction, double normalError, boolean growthAllowed, boolean inAlternate) {
        ARFFIMTDDforSOKNL.Node currentNode = node;
        while (true) {
            if (currentNode instanceof ARFFIMTDDforSOKNL.LeafNode) {
                // YS     Add Instances into LeafNodes
//                for(int i = 0; i < inst.weight(); i++) ((LeafNode) currentNode).leafInstances.add(inst);


                ((ARFFIMTDDforSOKNL.LeafNode) currentNode).learnFromInstance(inst, growthAllowed, this, (ARFFIMTDDforSOKNL.LeafNode)currentNode);

                break;
            } else {
                currentNode.examplesSeen += inst.weight();
                currentNode.sumOfAbsErrors += inst.weight() * normalError;
                ARFFIMTDDforSOKNL.SplitNode iNode = (ARFFIMTDDforSOKNL.SplitNode) currentNode;
                if (!inAlternate && iNode.alternateTree != null) {
                    ...
                    if (iNode.lossExamplesSeen - iNode.previousWeight >= alternateTreeTMinOption.getValue()) {
                        ...
                        if (Qi > 0) {
                            // Switch the subtrees
                            ARFFIMTDDforSOKNL.Node parent = currentNode.getParent();

                            if (parent != null) {
                                ARFFIMTDDforSOKNL.Node replacementTree = iNode.alternateTree;
                ...
                if (currentNode instanceof ARFFIMTDDforSOKNL.SplitNode) {
                    currentNode = ((ARFFIMTDDforSOKNL.SplitNode) currentNode).descendOneStep(inst);
                }
            }
        }
    }
    ...
    protected ARFFIMTDDforSOKNL.SplitNode newSplitNode(InstanceConditionalTest splitTest) {
        maxID++;
        return new ARFFIMTDDforSOKNL.SplitNode(splitTest, this);
    }

    protected ARFFIMTDDforSOKNL.LeafNode newLeafNode() {
        maxID++;
        return new ARFFIMTDDforSOKNL.LeafNode(this, this.subspaceSizeOption.getValue());
    }

    protected ARFFIMTDDforSOKNL.FIMTDDPerceptron newLeafModel() {
        return new ARFFIMTDDforSOKNL.FIMTDDPerceptron(this);
    }
    ...
    protected void checkRoot() {
        if (treeRoot == null) {
            ...
            leafNodes.add((ARFFIMTDDforSOKNL.LeafNode) treeRoot);
            leafNodeCount = 1;
        }
    }
    ...
    protected void attemptToSplit(ARFFIMTDDforSOKNL.LeafNode node, ARFFIMTDDforSOKNL.Node parent, int parentIndex) {
        ...
        // ine if any of the splits should be performed
        ...
        if (shouldSplit) {
            ...
            ARFFIMTDDforSOKNL.SplitNode newSplit = newSplitNode(splitDecision.splitTest);
            newSplit.copyStatistics(node);
            newSplit.changeDetection = node.changeDetection;
            newSplit.ID = node.ID;
            leafNodeCount--;
            for (int i = 0; i < splitDecision.numSplits(); i++) {
                ARFFIMTDDforSOKNL.LeafNode newChild = newLeafNode();
...
//            node.leafInstances = null;
            ...
            if (parent == null && node.originalNode == null) {
                ...
                ((ARFFIMTDDforSOKNL.SplitNode) parent).setChild(parentIndex, newSplit);
    ...
    public ARFFIMTDDforSOKNL.Node getLeafForInstance(Instance inst, ARFFIMTDDforSOKNL.Node node){
        if(node == null) return null;

        if(node instanceof ARFFIMTDDforSOKNL.LeafNode) return node;
        else{
            ARFFIMTDDforSOKNL.SplitNode currentNode = (ARFFIMTDDforSOKNL.SplitNode)node;
            return getLeafForInstance(inst,currentNode.children.get(currentNode.splitTest.branchForInstance(inst)));
        }
    }

    public ARFFIMTDDforSOKNL.Node getTreeRoot(){
