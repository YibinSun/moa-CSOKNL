...
import moa.classifiers.trees.ARFFIMTDDforKNNinLeaves;
...
/**
 * Implementation of ARFKNNinLeaves, an extension of AdaptiveRandomForest for classification.
 *
 ...
 */
public class ARFKNNinLeaves extends AbstractClassifier implements Regressor{
    ...
    /**
     * YS      Occupied Characters for Options: l s o m a x p u q i c k n t r
     **/

    public ClassOption treeLearnerOption = new ClassOption("treeLearner", 'l',
            "Random Forest Tree.", ARFFIMTDDforKNNinLeaves.class,
            "ARFFIMTDDforKNNinLeaves -s VarianceReductionSplitCriterion -g 50 -c 0.01");
    ...
    public IntOption kLeavesOption = new IntOption("NumberOfLeaves", 'k', "Specify how many leaves are being used", 5, 1, 10000);
    ...
    protected ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner[] ensemble;
    ...
    public void resetLearningImpl() {
        ...
        // YS     Initiate the random seed
        this.classifierRandom = new Random(randomSeedOption.getValue());

        // YS     Reset the evaluator array
        this.evaluators = new BasicRegressionPerformanceEvaluator[this.kLeavesOption.getValue()];
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        // YS     Update the evaluators by previous predictions
        InstanceExample example = new InstanceExample(instance);
        for (int i = 0; i < this.evaluators.length; i++) {
            this.evaluators[i].addResult(example, new double[]{this.previousPredictions[i]});
        }
    }
...
//            int k = this.kLeavesOption.getValue();
//            ArrayList<ARFFIMTDDforKNNinLeaves.LeafNode> candidates = new ArrayList<>();
//
//            for (ARFFIMTDDforKNNinLeavesBaseLearner a : this.ensemble) if(a.classifier != null) candidates.add((ARFFIMTDDforKNNinLeaves.LeafNode) a.classifier.getLeafForInstance(instance, a.classifier.getTreeRoot()));
//
...
//                    for(Instance inst : candidates.get(i).leafInstances) {
//                        distances[i] += getDistance(inst,instance)/candidates.get(i).leafInstances.size();
//                        treePredictions[i] += inst.classValue()/candidates.get(i).leafInstances.size();
//                    }
//                }
//            }
//            double prediction = 0;
...
//                    if (candidates.get(i) != null) for (Instance inst : candidates.get(i).leafInstances)
//                        prediction += inst.classValue() / (candidates.get(i).leafInstances.size() * k);
//            }
//
//
//            return new double[] {prediction};
            ...
            // KNN in Leaves
            // Pick the prestored prediction for new instance if it has been processed in the last 1000 instances.
...
//            InstanceExample example = new InstanceExample(instance);
            ...
            for (ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner a : this.ensemble)
                if (a.classifier != null)
                    candidates.add((ARFFIMTDDforKNNinLeaves.LeafNode) a.classifier.getLeafForInstance(instance, a.classifier.getTreeRoot()));

//            double[] predics = new double[candidates.size()];

            HashMap distancesFromEachInstance = new HashMap();
            // Update distances in the HashMap and the predictions in all candidates

            for (ARFFIMTDDforKNNinLeaves.LeafNode l : candidates) if (l != null) for (Instance ins : l.leafInstances) if (!distancesFromEachInstance.containsKey(ins.hashCode())) distancesFromEachInstance.put(ins.hashCode(), getDistance(ins, instance));

            double[] performances = new double[k];
//            double[] predics = new double[k];

            // Reset the previous prediction to 0
            this.previousPredictions = new double[this.previousPredictions.length];

            for (int i = 0; i < k; i++) {
                for(ARFFIMTDDforKNNinLeaves.LeafNode l : candidates){
                    if(l != null) {
                        if (k < l.leafInstances.size()) {
                            double[] distances = new double[l.leafInstances.size()];
                            for (int j = 0; j < l.leafInstances.size(); j++) {
                                distances[j] = (double) distancesFromEachInstance.get(l.leafInstances.get(j).hashCode());
                            }
                            int[] indices = indicesOfKSmallestValues(distances, i + 1);

                            for (Integer n : indices) this.previousPredictions[i] += l.leafInstances.get(n).classValue() / (i + 1) / 100;

                        } else {
                            this.previousPredictions[i] += l.getPrediction(instance) / 100;
                        }
                    }
                }
            }
            for(int e = 0; e < this.evaluators.length; e++) performances[e] = this.evaluators[e].getSquareError();

            return new double[] {this.previousPredictions[indexOfSmallestValue(performances)]};
//            for (ARFFIMTDDforKNNinLeaves.LeafNode l : candidates){
//                double[] distances = new double[l.leafInstances.size()];
//                for(int n = 0; n < l.leafInstances.size(); n++){
//                    distances[n] = (double)distancesFromEachInstance.get(l.leafInstances.get(n).hashCode());
//                }
//                int[] indices = indicesOfKSmallestValues(distances, k);
//                for (int i = 0; i < k; i++) {
//                    predics[i] =
//                }
//                for (Instance ins : l.leafInstances){
//
//
//                }
//
//
//
//                distances[j] = (double) distancesFromEachInstance.get(ins.hashCode());
//            }
//            int[] indices = indicesOfKSmallestValues(distances, k);
//            for (Integer in : indices) {
//                predics[i] = l.leafInstances.get(in).classValue() / k;
//
//            }
//
//            double prediction = 0;
//            for (Double d : predics) prediction += d / predics.length;
//
//            return new double[]{prediction};
        }
...
//        }
    ...
    protected void initEnsemble(Instance instance) {
        ...
        this.ensemble = new ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner[ensembleSize];
        ...
        ARFFIMTDDforKNNinLeaves treeLearner = (ARFFIMTDDforKNNinLeaves) getPreparedClassOption(this.treeLearnerOption);
        treeLearner.resetLearning();

        for (int i = 0; i < ensembleSize; ++i) {
            treeLearner.subspaceSizeOption.setValue(this.subspaceSize);
            this.ensemble[i] = new ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner(
                    i,
                    (ARFFIMTDDforKNNinLeaves) treeLearner.copy(),
    ...
    protected final class ARFFIMTDDforKNNinLeavesBaseLearner extends AbstractMOAObject {
        public int indexOriginal;
        public long createdOn;
        public long lastDriftOn;
        public long lastWarningOn;
        public ARFFIMTDDforKNNinLeaves classifier;
        ...
        // Bkg learner
        protected ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner bkgLearner;
        // Statistics
        ...
        private void init(int indexOriginal, ARFFIMTDDforKNNinLeaves instantiatedClassifier, BasicRegressionPerformanceEvaluator evaluatorInstantiated,
            ...
            // YS   Pass the random seed to ARFFIMTDDforKNNinLeaves
            this.classifier.classifierRandom = random;
        }

        public ARFFIMTDDforKNNinLeavesBaseLearner(int indexOriginal, ARFFIMTDDforKNNinLeaves instantiatedClassifier, BasicRegressionPerformanceEvaluator evaluatorInstantiated,
                                    long instancesSeen, boolean useBkgLearner, boolean useDriftDetector, ClassOption driftOption, ClassOption warningOption, boolean isBackgroundLearner, Random random) {
...
//                boolean correctlyClassifies = this.classifier.correctlyClassifies(instance);
                ...
                if (this.useBkgLearner) {
                    ...
                    if (this.warningDetectionMethod.getChange()) {
                        ...
                        ARFFIMTDDforKNNinLeaves bkgClassifier = (ARFFIMTDDforKNNinLeaves) this.classifier.copy();
                        ...
                        this.bkgLearner = new ARFKNNinLeaves.ARFFIMTDDforKNNinLeavesBaseLearner(indexOriginal, bkgClassifier, bkgEvaluator, instancesSeen,
